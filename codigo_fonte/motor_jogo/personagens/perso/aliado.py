...
class Aliado(Personagem):
    def __init__( _ , classe_aliada : str , posicao_roteiro : str , magia ):
        super().__init__()

        caminho_comum : str  = Linkzin( getcwd() , 'banco_dados' , 'Personagens' , 'aliados' )
        bifurcacao    : str  = Linkzin( caminho_comum , classe_aliada ) # caveira , Lady Medusa , ...
        ...
        with open(Linkzin( getcwd() , 'roteiro' , 'status_personagens.json' ) , 'r' ) as status:
            ...
            status_perso  = status_totais[0][posicao_roteiro]
        ...
        with open(Linkzin( getcwd() , 'roteiro' , 'dialogos_perso.json') , 'r' , encoding="utf-8" ) as status:
            _.todas_falas = carregar_json(status)[0][posicao_roteiro]
    ...
    def repr_aliado( _ , genero : str , texto_convencional: str , nome_derrota: str ):
        return super().repr_personagem( f'{genero} {texto_convencional}' , f'{genero} {nome_derrota} caiu :(' , ( "Saudavel!" , f"Ferid{genero.lower()}, mas de pé!" ))
